...
# -*- coding: utf-8 -*-
"""
Diagnostics core — обновлено (совместимость API сохранена).
- Имена/сигнатуры функций прежние.
- Улучшения:
  * Геометрия подшипника: поддержка расчёта BPFI/BPFO/BSF/FTF из N,d,D,θ;
    также как раньше — коэффициенты (<10) или абсолютные Гц (>=10).
  * f5 теперь считается из PSD тока как индикатор боковых f1±f_r (раньше — по огибающей).
  * Роторная ветка допускается и при 1 канале (если ALLOW_SINGLE_PHASE_ROTOR=True).
  * Broken bars: динамический верхний предел δ по max(ROTOR_BB_MAX_OFFSET, 0.06·f1).
  * Убрано дублирование BearingSet/_bearing_family.
  * Мелкие численные улучшения и защита от краевых случаев.

Формат комментариев (docstring) везде одинаковый:
- Args: входные параметры (единицы измерения/ожидания)
- Returns: формат результата
- Rationale: краткое пояснение математической модели/решения
"""
...
DEFAULT_RPM: float = 1500.0

# === Вспомогательные фильтры/БПФ ===

def _safe_norm_freq(fc, fs):
    ny = 0.5 * fs
    def _norm(c):
        try:
            w = _safe_norm_freq(fc, fs)
        except Exception:
            w = 0.5
        if w <= 0: w = 1e-6
        if w >= 1: w = 0.999
    ...
    if isinstance(fc, (tuple, list)):
        lo, hi = sorted((_norm(fc[0]), _norm(fc[1])))
        lo = max(1e-5, lo); hi = min(0.999, hi)
        return (lo, hi)
    return _norm(fc)

def _butter(x: NDArray[np.float64], fs: float, fc: float | tuple, btype: str, order: int = 2) -> NDArray[np.float64]:
    """Фильтрация (Butterworth) с нулевой фазой.

    Args:
        x: временной ряд (A), numpy-вещественный массив.
        fs: частота дискретизации, Гц (> 2*max(fc)).
        fc: частота(ы) среза (Гц). Для полосы — кортеж (low, high).
        btype: тип фильтра ('highpass'|'lowpass'|'bandpass'|'bandstop').
        order: порядок фильтра (по умолчанию 2).

    Returns:
        Отфильтрованный сигнал такой же длины, как x.

    Rationale:
        Применяется Butterworth + filtfilt для нулевого сдвига фазы (важно для фазового
        анализа и корректной огибающей).
    """
    x = np.asarray(x, dtype=float)
    if x.size == 0:
        return x
    ny = max(1e-6, 0.5 * fs)
    w = fc / ny if not isinstance(fc, (tuple, list)) else [c / ny for c in fc]
...
class BearingSet:
    ...
    BPFI: float; BPFO: float; BSF: float; FTF: float
...
def _bearing_family(shaft_hz: float) -> BearingSet:
    ...
    Rationale:
        ...
        При отсутствии геометрии используем типичные коэффициенты.
    """
    cfg = _load_bearing_config()
    if cfg:
        N = cfg.get("N")
        D = cfg.get("D")
        d = cfg.get("D_BALL", cfg.get("D_B", cfg.get("D_BALLS", cfg.get("D_BALL_DIAM", cfg.get("D_BALL_DIAMETER", cfg.get("D_BALL_D", cfg.get("D_BALL_MM", cfg.get("D_BALL_DIAM_MM", cfg.get("D_BALLS_DIAM", cfg.get("D_BALLS_DIAMETER", cfg.get("D_BALLS_MM", cfg.get("D_BALLS_D"))))))))))))
        if d is None:
        ...
        if have_any:
            return BearingSet(vals.get("BPFI", 5.4*shaft_hz),
                              vals.get("BPFO", 4.6*shaft_hz),
                              vals.get("BSF", 2.0*shaft_hz),
                              vals.get("FTF", 0.4*shaft_hz))
    return BearingSet(5.4 * shaft_hz, 4.6 * shaft_hz, 2.0 * shaft_hz, 0.4 * shaft_hz)
...
def _family_score_env(f: NDArray[np.float64], A: NDArray[np.float64], fr: float, fam_hz: float) -> float:
    ...
    """
    if fam_hz <= 0 or f.size == 0 or A.size == 0:
        return 0.0
    def band_energy(f0: float, width: float, harmonics=(1,2,3), sidebands: Iterable[float]=(-float(fr), float(fr))):
        total = 0.0
        for h in harmonics:
            fc = f0 * h
            m = (f >= fc - width) & (f <= fc + width)
            total += float(np.max(A[m]) if np.any(m) else 0.0)
            for sb in sidebands:
                fsb = fc + sb
                m = (f >= fsb - width) & (f <= fsb + width)
                total += float(np.max(A[m]) if np.any(m) else 0.0)
        return total
    width = max(1.5, 0.10*fr)
    energy = band_energy(fam_hz, width)
    win, gap = max(10.0, 0.5*fr), max(1.0, 0.1*fr)
    left  = (f >= fam_hz - win - gap) & (f < fam_hz - gap)
    right = (f > fam_hz + gap) & (f <= fam_hz + win + gap)
    noise = float(np.median(np.hstack([A[left], A[right]]))) if (np.any(left) or np.any(right)) else 1e-6
    return 20.0 * math.log10((energy + 1e-9) / (noise + 1e-9))

# === MCSA: сила боковых линий f1 ± m·f_fault в PSD тока ===
...
def _mcsa_sidebands_score(f_psd: NDArray[np.float64], Pxx: NDArray[np.float64],
    ...
    """Скоринг MCSA для подшипников: боковые f1±m·f_fault в PSD тока.

    Args:
        f_psd: частоты PSD (Гц) из Welch.
        Pxx: плотность мощности (A^2/Hz, условно), тот же размер.
        mains: частота сети f1 (Гц).
        side_freq: частота дефекта (BPFI/BPFO/BSF/FTF) в Гц.
        multipliers: набор m (кратности), по умолчанию (1,2).
        bw: полуширина окна поиска пика (Гц).

    Returns:
        Балл в дБ относительно медианного шума над f1.

    Rationale:
        Дефект подшипника амплитудно модулирует ток с частотой f_fault → боковые линии возле f1.
    """
...
# === Автоподбор частоты вала ===

def _autoselect_shaft_hz(env_f: NDArray[np.float64], env_A: NDArray[np.float64], rpm_guess: float) -> float:
    """Оценка частоты вала по максимуму суммарного семейного скоринга.

    Args:
        env_f, env_A: спектр огибающей (Гц, амплитуда).
        rpm_guess: грубая оценка оборотов, RPM.

    Returns:
        Оценка fr (Гц).

    Rationale:
        Выбирается fr в узком окне около rpm_guess/60, максимизирующий сумму скорингов BPFI/BPFO/BSF/FTF.
    """
    base = max(0.1, rpm_guess / 60.0)
    cand = np.linspace(0.9 * base, 1.1 * base, 11)
    def fam_total(fr_):
    ...
    i = int(np.argmax(scores)); fr_best = float(cand[i])
    if 0 < i < len(cand)-1:
        x = cand[i-1:i+2]; y = scores[i-1:i+2]
...
def get_feature_vector(curR: NDArray[np.float64], curS: NDArray[np.float64], curT: NDArray[np.float64],
    ...
    r = preprocess_per_phase(curR, fs); s = preprocess_per_phase(curS, fs); t = preprocess_per_phase(curT, fs)
    x = (r + s + t) / 3.0

    fe, Ae = envelope_spectrum_2_3k(x, fs)
    f_psd, Pxx = welch(x, fs=fs, nperseg=min(4096, len(x)))

    fr = _autoselect_shaft_hz(fe, Ae, rpm_guess)
    ...
    f1 = peak_at_env(fam.BPFI); f2 = peak_at_env(fam.BPFO); f3 = peak_at_env(fam.BSF); f4 = peak_at_env(fam.FTF)
    ...
    st_i = stats_around_env(fam.BPFI); st_o = stats_around_env(fam.BPFO); st_b = stats_around_env(fam.BSF); st_c = stats_around_env(fam.FTF)
    ...
    if np.count_nonzero(valid) > 10:
        X = np.log10(fe[valid]); y = np.log10(Ae[valid])
...
def classify_defect_scored(curR: NDArray[np.float64], curS: NDArray[np.float64], curT: NDArray[np.float64],
                           fs: float = DEFAULT_FS, rpm_guess: float = DEFAULT_RPM) -> Tuple[str, str, float, Optional[str]]:
    """Классификатор с баллами: подшипник/ротор/несоосность/норма.
    ...
    Rationale:
        ...
        Если неубедительно — ветка ротора/несоосности по SNR@f1, симметричной паре около f1 и боковым f1±m·f_r.
    """
    mains = CONFIG["MAINS_HZ"]
    r = preprocess_per_phase(curR, fs); s = preprocess_per_phase(curS, fs); t = preprocess_per_phase(curT, fs)
    x = (r + s + t) / 3.0
    ...
    if multiphase or allow_single:
        ...
        def max_coh(a, b):
            fcoh, Cxy = coherence(a, b, fs=fs, nperseg=min(4096, len(a)))
        ...
        if multiphase:
            ...
            coh_ok = (coh >= CONFIG["COH_MSC_MIN"])  # старое правило
        else:
            coh = 1.0
            coh_ok = True
        ...
        if (snr_m >= CONFIG["ROTOR_SNR_T"]) and coh_ok:
            ...
            if a100_a50 >= CONFIG["A100_A50_MIN"]:
                return "Misalignment", severity_from_K(snr_m/2.0), snr_m, None
            else:
